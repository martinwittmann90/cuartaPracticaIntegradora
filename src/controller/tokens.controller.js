...
import { mailController } from '../controller/messageandmails.controller.js';
...
class TokenController {
  ...
  async sendPasswordResetEmail(req, res) {
    try {
      ...
      const user = await serviceUsers.getUserByEmailService(trimmedEmail);
      logger.debug(user);
      if (!user) {
  ...
  async postPassword(req, res) {
    try {
      ...
      const user = await serviceUsers.getUserByEmailService(email);
      if (!user) {
        return res.status(404).json({ success: false, message: 'User not found' });
      }
