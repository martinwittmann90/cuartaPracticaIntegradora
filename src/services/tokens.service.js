...
class TokenService {
  async generateResetTokenService(userId) {
    try {
      ...
      const newToken = await tokenDAO.createTokenDao(tokenData);
  ...
  async isResetTokenValidService(tokenNumber) {
    try {
      const tokenFinderOnDatabase = await tokenDAO.findTokenByTokenDao(tokenNumber);
